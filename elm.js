...
var $author$project$Main$emptyScore = A3(
	...
	A3(
		...
		A3(
			...
			'Nordan',
			0,
			A3($elm$core$Dict$insert, 'Östan', 0, $elm$core$Dict$empty))));
var $author$project$Main$init = {winds: $author$project$Main$emptyScore, winner: ''};
...
var $author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'ChangeScore') {
			var name = msg.a;
			var scoreString = msg.b;
			var _v1 = $elm$core$String$toInt(scoreString);
			if (_v1.$ === 'Just') {
				var score = _v1.a;
				return _Utils_update(
					model,
					{
						winds: A3($elm$core$Dict$insert, name, score, model.winds)
					});
			} else {
				return _Utils_update(
					model,
					{
						winds: A3($elm$core$Dict$insert, name, 0, model.winds)
					});
			}
		} else {
			var name = msg.a;
...
var $author$project$Main$take = F4(
	function (amount, from, to, scores) {
		var amount_ = ((from === 'Östan') || (to === 'Östan')) ? (amount * 2) : amount;
		var _v0 = A2(
			$elm$core$Debug$log,
			'take',
			_Utils_Tuple3(amount, from, to));
		return A3(
			$elm$core$Dict$update,
			to,
			$elm$core$Maybe$map(
				function (score) {
					return score + amount_;
				}),
			A3(
				$elm$core$Dict$update,
				from,
...
var $author$project$Main$do = F2(
	function (winds, scores) {
		if (winds.b) {
			var to = winds.a;
			var others = winds.b;
			var transact_ = F2(
				function (_v1, scores_) {
					var from = _v1.a;
					var points = _v1.b;
					return A4($author$project$Main$take, to.b - points, from, to.a, scores_);
				});
			return A2(
				$author$project$Main$do,
				others,
				A3($elm$core$List$foldl, transact_, scores, others));
		} else {
			return scores;
		}
	});
var $elm$core$List$sortBy = _List_sortBy;
var $author$project$Main$calculateScores = function (model) {
	var winnerPoints = A2(
		$elm$core$Maybe$withDefault,
		0,
		A2($elm$core$Dict$get, model.winner, model.winds));
	var others = $elm$core$List$reverse(
		A2(
			...
			A2(
				...
				A2(
					...
					$elm$core$Basics$neq(model.winner)),
				$elm$core$Dict$toList(model.winds))));
	return A2(
		$author$project$Main$do,
		others,
		A3(
			...
			F2(
				function (_v0, scores) {
					var name = _v0.a;
					return A4($author$project$Main$take, winnerPoints, name, model.winner, scores);
				}),
			$author$project$Main$emptyScore,
			others));
};
...
var $author$project$Main$ChangeScore = F2(
	function (a, b) {
		return {$: 'ChangeScore', a: a, b: b};
	});
var $author$project$Main$ChangeWinner = function (a) {
	return {$: 'ChangeWinner', a: a};
};
...
var $author$project$Main$viewWind = F4(
	function (winner, scores, name, points) {
		return A2(
						...
						[
							...
							_List_fromArray(
								[
									...
									$rtfeldman$elm_css$Html$Styled$Attributes$name('winner'),
									$rtfeldman$elm_css$Html$Styled$Attributes$value(name),
									$rtfeldman$elm_css$Html$Styled$Events$onInput($author$project$Main$ChangeWinner),
									$rtfeldman$elm_css$Html$Styled$Attributes$checked(
									_Utils_eq(winner, name))
								]),
							_List_Nil)
						])),
					A2(
						...
						[
							...
							_List_fromArray(
								[
									...
									(!points) ? '' : $elm$core$String$fromInt(points)),
									$rtfeldman$elm_css$Html$Styled$Events$onInput(
									$author$project$Main$ChangeScore(name)),
						...
						$rtfeldman$elm_css$Html$Styled$text(
							A2(
								...
								A2(
									...
									A2($elm$core$Dict$get, name, scores))))))
				]));
	});
var $author$project$Main$view = function (model) {
	var scores = $author$project$Main$calculateScores(model);
	var winds = $elm$core$Dict$values(
		A2(
			$elm$core$Dict$map,
			A2($author$project$Main$viewWind, model.winner, scores),
			model.winds));
	var headers = A2(
		$rtfeldman$elm_css$Html$Styled$tr,
		_List_fromArray(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('Mahjong')
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$td,
				_List_Nil,
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('Poäng')
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$td,
				_List_Nil,
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('Resultat')
					]))
			]));
	return A2(
		...
		A2($elm$core$List$cons, headers, winds));
};
